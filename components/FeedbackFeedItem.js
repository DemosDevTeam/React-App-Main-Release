...
export default class FeedbackFeedItem extends React.Component {
    constructor(props) {
      super(props);
    ...
    componentDidMount = async () => {
      ...
      await database.ref('/Users/' + user + '/Reactions/' + articleId + '/').once('value').then(snap => {
        console.log("snap is ");
        console.log(snap.val());
        if(snap.hasChild('answers')){
          console.log("inside of snap has child answers");
          if(snap.child('answers').hasChild('frquestions') && snap.child('answers').hasChild('mcquestions')){
            console.log("inside of snap has frques and mcques");
          ...
          } else if(snap.child('answers').hasChild('frquestions')) {
            snap.child('answers').child('frquestions').forEach(question => {
              feedback.push(
                <View>
                  <View>
          ...
          } else if(snap.child('answers').hasChild('mcquestions')) {
            snap.child('answers').child('mcquestions').forEach(question => {
              feedback.push(
                <View>
                  <View>
